...
public class GameScript : MonoBehaviour {
    ...
    public Image bg;
    private bool trueColor = false, falseColor = false,defaultColor = false;
    private int randQ;
    ...
    void generateQuestion()
    {
        if (qList.Count > 0)
        {
            ...
            questionText.GetComponent<Animation>().Play("Bubble_Open_2");
    ...
    {
        answerClicked = true;
        StartCoroutine(trueOrFalse(answersText[index].text.ToString() == crntQ.answers[0] ? true : false));
    }
    IEnumerator final()
    {
    ...
    IEnumerator timer()
    {
        ...
        for (int a = 0; a < answerBttns.Length; a++) answerBttns[a].interactable = false;
    ...
    IEnumerator answersBttnsInAnim()
    {
        ...
        for (int a = 0; a < answerBttns.Length; a++) answerBttns[a].interactable = true;
        yield return StartCoroutine(timer());
    }
    IEnumerator timeOut()
    {
        for (int i = 0; i < answerBttns.Length; i++) answerBttns[i].GetComponent<Animation>().Play("Bubble_Close_2");
        falseColor = true;
        ...
        questionText.GetComponent<Animation>().Play("Bubble_Close_2");
        yield return new WaitForSeconds(0.5f);
